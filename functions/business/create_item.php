<?php

include("../connect.php");
session_start();
$digits = 7;
$randms = str_pad(rand(0, pow(10, $digits) - 1), $digits, '0', STR_PAD_LEFT);
$today = date('Y-m-d H:i:s');

if (isset($_POST['name']) && isset($_POST['qty']) && isset($_POST['price']) && isset($_POST['code'])) {
    // intialize post values
    $name = $_POST['name'];
    $unit_price = floatval(preg_replace('/[^\d.]/', '', $_POST['price']));
    $quantity = $_POST['qty'];
    $code = $_POST['code'];

    $itemData = [
        'code' => $code,
        'name' => $name,
        'unitPrice' => $unit_price,
        'quantity' => $quantity,
        'description' => $_POST['description'],
        'dateAdded' => $today
    ];
    $addItem = insert('items', $itemData);
    if (!$addItem) {
        $error = "Error: \n" . mysqli_error($connection); //checking for errors
        $_SESSION["feedback"] = "Sorry could not add Item! - $error";
        $_SESSION["Lack_of_intfund_$randms"] = "10";
        echo header("Location: ../../inventory.php?message1=$randms");
        exit();
    } else {
        $_SESSION["feedback"] = "Item added successfully";
        $_SESSION["Lack_of_intfund_$randms"] = "10";
        echo header("Location: ../../inventory.php?message0=$randms");
        exit();
    }
} else {
    // fill in all appropraite data
    $_SESSION["feedback"] = "Fill in all Neccessary Fields";
    $_SESSION["Lack_of_intfund_$randms"] = "10";
    echo header("Location: ../../inventory.php?message1=$randms");
    exit();
}
